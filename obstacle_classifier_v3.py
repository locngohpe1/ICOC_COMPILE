# obstacle_classifier_v3.py - Simple version with dropout
import torch
...
class ObstacleClassifierV3:
    def __init__(self, use_pretrained=True, use_gpu=True):
        ...
        model_path = "models/googlenet_obstacle_classifier_v3.pth"
    ...
    def train(self, train_dataloader, val_dataloader, num_epochs=100, learning_rate=0.001):
        ...
        for epoch in range(num_epochs):
            ...
            if val_acc > best_val_acc:
                ...
                if not os.path.exists('models'):
                    os.makedirs('models')
                torch.save(self.model.state_dict(), "models/googlenet_obstacle_classifier_v3_best.pth")
            else:
                patience_counter += 1
        ...
        # Restore best
        best_path = "models/googlenet_obstacle_classifier_v3_best.pth"
        if os.path.exists(best_path):
            self.model.load_state_dict(torch.load(best_path, map_location=self.device))
            print(f"\n✅ Restored best model ({best_val_acc * 100:.2f}%)")

        # Save
        torch.save(self.model.state_dict(), "models/googlenet_obstacle_classifier_v3.pth")
        print(f"✅ Saved to: models/googlenet_obstacle_classifier_v3.pth")

        df = pd.DataFrame(training_history)
        df.to_csv('models/training_history_v3.csv', index=False)
